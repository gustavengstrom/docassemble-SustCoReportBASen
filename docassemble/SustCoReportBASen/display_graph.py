...
def show_graph_scatter(df, filename, visualization_image, height, width):
    ...
    abbr = {
        ...
        "Labour practices": "C",
        "Environment": "D",
        "Fair operating practices": "E",
        "Consumer issues": "F",
        "Community involvement and development": "G",
    }
    ...
    df_group = pd.DataFrame(
        df.groupby(
            [
                "Current impact on core subjects",
                "Importance of core subjects for strategic goals",
            ]
        )
        .apply(lambda x: x["Core subject area"].to_list())
        .reset_index()
    )
    ...
    df_group["Core subject area"] = df_group["Ansvarsområde_List"].apply(
        lambda x: "<br>".join(x)
    )
    ...
    fig = px.scatter(
        df_group,
        x="Current impact on core subjects",
        y="Importance of core subjects for strategic goals",
        size="size",
        size_max=15,
        labels="abbr_text",
        color="Core subject area",
        symbol="Core subject area",
    ...
    fig.update_xaxes(range=[-0.1, 1.1], ticktext=["Negative impact", "Positive impact"],
    tickvals=[0.2, 0.8,],)
    fig.update_yaxes(range=[-0.1, 1.1], ticktext=["Low importance", "High importance"],
    tickvals=[0.2, 0.8,],)
